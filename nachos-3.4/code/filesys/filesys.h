...
public:
	...
	void SetOpenFile(int index, OpenFile* openFile)
	{
		if (index < 0 || index >= MAX_OPEN_FILE)
		{
			return;
...
public:
	...
	int FindFreeSlot(); // find a free slot in openFileTable

	void SetOpenFile(int index, OpenFile *openFile); // set open file to openFileTable

	OpenFile *GetOpenFile(int index); // get open file from openFileTable

	void CloseOpenFile(int index); // close open file from openFileTable
...
private:
	...
	OpenFile **openFileTable; // Open file table
};
